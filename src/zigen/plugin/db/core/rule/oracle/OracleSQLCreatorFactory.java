...
/**
 * OracleInsertFactory.javaÉNÉâÉX.
 * 
 * @author ZIGEN
 * @version 1.0
...
public class OracleSQLCreatorFactory extends DefaultSQLCreatorFactory {
	...
	/**
	 * ÉJÉâÉÄñºÅ{å^Å{åÖ(OracleópÇ…Defaultï\é¶Çí«â¡)
	 * 
	 * @return
	 */
	protected String getColumnLabel(TableColumn column) {
		...
		// ÉpÉâÉÅÅ[É^ñ≥ÇµÇÃå^ëŒâû(ó· NUMBERå^)
	...
	public String createSelect(String _condition, int limit) {
		...
		if (condition != null && !"".equals(condition.trim())) { //$NON-NLS-1$
			...
			if (limit > 0) {
				sb.append(" AND ROWNUM <= " + (++limit));// É_ÉCÉAÉçÉOÇèoÇ∑ÇΩÇﬂÇ…Å{ÇP
				// //$NON-NLS-1$
			}
		} else {
			if (limit > 0) {
				sb.append(" WHERE ROWNUM <= " + (++limit));// É_ÉCÉAÉçÉOÇèoÇ∑ÇΩÇﬂÇ…Å{ÇP
				// //$NON-NLS-1$
			}
		}
	...
	public String createSelectForPager(String _condition, int offset, int limit) {
		...
		//sb.append(") W ) T WHERE line BETWEEN ").append(offset).append(" AND ").append(offset + limit);	// MaxReocordExceptionÇÇ†Ç¶Çƒî≠ê∂Ç∑ÇÈèÍçá
...
//	/**
//	 * OralceÇ≈ÇÕÅAUniqueIndexÇDDLÇ…ä‹ÇﬂÇ»Ç¢
//	 */
